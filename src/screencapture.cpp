...
#define CURRENTTIMES  QDateTime::currentDateTime().toString("yyyyMMddHHmmss")
...
void ScreenCapture::initGeometry(const int offsetX, const int offsetY, const int width, const int height)
{
    m_offsetX = offsetX;
    m_offsetY = offsetY;
    m_width = width;
...
void ScreenCapture::run()
{
    ...
    int err = 0;
    m_bStop = false;
    fileName = qApp->applicationDirPath() + "/resource/video" + "/"
             + CURRENTTIMES + "ScreenCap" + ".mp4";
    ...
    while (!m_bStop)
    {
        ...
        if ((av_read_frame(ifmt_ctx, avDePacket)) >= 0)
        {
            ...
            {
                //+
                int retu = avcodec_send_packet(deCodecCtx,avDePacket);
                while(retu >= 0)
                {
                    retu = avcodec_receive_frame(deCodecCtx,avFrameRgb);
...
bool ScreenCapture::intiDeContext()
{
    ...
    /* 指定录屏的分辨率 */
    av_dict_set(&options,"offset_x",QString::number(m_offsetX).toLatin1().data(),0);
    av_dict_set(&options,"offset_y",QString::number(m_offsetY).toLatin1().data(),0);
    av_dict_set(&options, "video_size", QString("%1x%2").arg(m_width).arg(m_height).toLatin1().data(), 0);

    av_dict_set(&options, "framerate","30", 0);
    av_dict_set(&options, "pix_fmt","yuv420p", 0);
    //设置以最快的速度进行编码+
    av_dict_set(&options,"preset","ultrafast",0);
    av_dict_set(&options,"vcodec","h264_nvenc",0);
    //获取存放视频文件的抽象容器
    ...
    /* 获取流信息 */
    int result = avformat_find_stream_info(ifmt_ctx, NULL);
    if (result < 0) {
    ...
    int num = in_stream->avg_frame_rate.num;
    int den = in_stream->avg_frame_rate.den;
    if (num != 0 && den != 0) {
        videoFps = num / den;
    }
    QString videoInfo = QString("保存视频流信息 -> 索引: %1  解码: %2  格式: %3  时长: %4 秒  fps: %5  分辨率: %6*%7  rtsp地址: %8")
            .arg(videoDecoder->name)
            .arg(ifmt_ctx->iformat->name)
            .arg((ifmt_ctx->duration) / 1000000)
            .arg(videoFps)
            .arg(videoWidth)
            .arg(videoHeight)
            .arg(url);
